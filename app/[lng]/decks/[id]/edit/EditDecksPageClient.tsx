// app/[lng]/decks/[id]/edit/EditDecksPageClient.tsx
'use client';
...
interface EditDecksPageClientProps {
    lng: string;
}

const EditDecksPageClient: React.FC<EditDecksPageClientProps> = ({ lng }) => {
    ...
    const { id } = params;
    const [deckUser, setDeckUser] = useState<DeckUser | null>(null);
    ...
    const [selectedDeck, setSelectedDeck] = useLocalStorage<Card[]>('selectedDeckEdit', []);
    ...
    const handleSaveDeck = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch(`/api/decks/user/${id}/edit`, {
                method: 'PUT',
                headers: {
                ...
                body: JSON.stringify({
                    deckCards,
                    deckName: trimmedDeckName || t('common:notification.notification_unnamed'),
                }),
            });
    ...
    useEffect(() => {
        if (!session) {
            setError(t('common:notification.error_unauthorized'));
            return;
        }
        ...
        if (allCards.length === 0) {
            setError(t('common:notification.error_card_data_not_loaded'));
            return;
        }

        const fetchDeckUser = async () => {
            try {
                const response = await fetch(`/api/decks/user/${id}/edit`, {
                    method: 'GET',
                ...
                const data: DeckUser = await response.json();
                setDeckUser(data);

                // 將 deckCards 轉換為卡片號碼陣列
                const cardNumbers = data.deck.deckCards.split(',').map(num => num.trim());

                // 根據卡片號碼找到對應的卡片，允許重複
                const matchedCards: Card[] = cardNumbers.map(number => {
                    const foundCard = cardMap.get(number);
                    if (foundCard) {
                        return { ...foundCard };
                    } else {
                        // console.warn(`未找到卡片號碼: ${number}`);
                        return null;
                    }
                }).filter(card => card !== null) as Card[];

                setDeckName(data.deckName);
                setSelectedDeck(matchedCards);
            } catch (err) {
                // console.error('Error fetching DeckUser:', err);
...
export default EditDecksPageClient;